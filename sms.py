...
def Banner():
    ...
    banner ="""             
     © Copyright HUYCHUALAMBO

    IIIIIIII   IIIIIIIIII    IIIIIIII    II
       II      II      II    I     II    II
       II      II      II    I     II    II
       II      IT      II    I     II    II
       II      IIIIIIIIII    IIIIIIII    IIIIIIIIII
                               spam sms v1.0
╔════════════════════════════════════════════════════════╗ 
 => Author: HUY CHUA LAM BO
 TOOL LỎ SPAM HƠI YẾU
══════════════════════════════════════════════════════════
...
def oldpops(phone):
    ...
    if response.status_code == 200:
        print(Colorate.Diagonal(Colors.yellow_to_green, f"""DONE 😎"""))
    else:
        print(Colorate.Diagonal(Colors.red_to_yellow, f"""FAIL 😶"""))
...
def Main():
    ...
    Banner()
    while True:
        phone = Write.Input("=> SDT NẠN NHÂN CẦN SPAM : ", Colors.green_to_yellow, interval=0.005)
        if phone in listsdt:
            print(Colorate.Diagonal(Colors.red_to_red, f"""=> BỐ MÀY LÀ ADMIN, MÀY THÍCH SPAM KHÔNG CON CHÓ?"""))
        else:
            if len(phone) == 10:
                break
            else:
                print(Colorate.Diagonal(Colors.red_to_red, f"""=> SỐ ĐIỆN THOẠI PHẢI = 10 SỐ!"""))
    a=int(input("Nhập Số Lần Spam: "))
    i=1         
    while i<a+1:
        print(Colorate.Diagonal(Colors.green_to_yellow, f"""=> SPAM LẦN: {i}"""))
        i=i+1
        oldpops(phone)
        oldtv360(phone)
        oldloship(phone)
        ...
        thantaioi(phone)
        atmonline(phone)
        print(Colorate.Diagonal(Colors.green_to_yellow, f"""=> DONE! """))
        sleep(10)
